...
public class PostResource {
    ...
    public Response addPost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        ...
        try {
            ...
            var validation = Validations.checkAddPostValidation(user, community, member, token, authToken, data);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response getPost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, @PathParam("postID") String postID) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        LOG.fine("Get post: " + authToken.username + " attempted to get post with id " + postID + " in the community with id " + communityID + ".");
        try {
            ...
            var validation = Validations.checkGetPostValidation(user, community, member, post, token, authToken);
    ...
    public Response editPost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        LOG.fine("Edit post: " + authToken.username + " attempted to edit post with id " + data.postID + "  to the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            Entity member = serverConstants.getCommunityMember(txn, communityID, authToken.username);
            Entity token = serverConstants.getToken(txn, authToken.username, authToken.tokenID);
            var validation = Validations.checkEditPostValidation(user, community, post, member, token, authToken, data);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response removePost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, @PathParam("postID") String postID) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        LOG.fine("Remove post: " + authToken.username + " attempted to remove post with id " + postID + " from the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            Entity member = serverConstants.getCommunityMember(txn, communityID, authToken.username);
            Entity token = serverConstants.getToken(txn, authToken.username, authToken.tokenID);
            var validation = Validations.checkRemovePostValidation(user, community, post, member, token, authToken);
            //TODO: make validations
            validation = Response.ok().build();
            if ( validation.getStatus() == Status.UNAUTHORIZED.getStatusCode() ) {
                serverConstants.removeToken(txn, authToken.username, authToken.tokenID);
                ...
                txn.commit();
                LOG.fine("Remove post: " + authToken.username + " removed post from the community with id " + communityID + ".");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response lockPost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        String lock = data.isLocked ? "lock" : "unlock";
        LOG.fine("Lock post: " + authToken.username + " attempted to " + lock + " post with id " + data.postID + " from the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            Entity member = serverConstants.getCommunityMember(txn, communityID, authToken.username);
            Entity token = serverConstants.getToken(txn, authToken.username, authToken.tokenID);
            var validation = Validations.checkLockPostValidation(user, community, post, member, token, authToken, data);
            //TODO: make validations
            validation = Response.ok().build();
            if ( validation.getStatus() == Status.UNAUTHORIZED.getStatusCode() ) {
                serverConstants.removeToken(txn, authToken.username, authToken.tokenID);
                ...
                post = Entity.newBuilder(post.getKey())
                    ...
                    .set("dislikes", post.getLong("dislikes"))
                    .set("comments", post.getLong("comments"))
                    .set("isLocked", data.isLocked != post.getBoolean("isLocked") ? data.isLocked : post.getBoolean("isLocked"))
                    .set("isPinned", post.getBoolean("isPinned"))
                    .set("pinDate", post.getTimestamp("pinDate"))
                    .build();
                txn.put(post);
                txn.commit();
                LOG.fine("Lock post: " + authToken.username + " " + lock + "ed post from the community with id " + communityID + ".");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response pinPost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        String pin = data.isPinned ? "pin" : "unpin";
        LOG.fine("Pin post: " + authToken.username + " attempted to " + pin + " post with id " + data.postID + " from the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            Entity member = serverConstants.getCommunityMember(txn, communityID, authToken.username);
            Entity token = serverConstants.getToken(txn, authToken.username, authToken.tokenID);
            var validation = Validations.checkPinPostValidation(user, community, post, member, token, authToken, data);
            //TODO: make validations
            validation = Response.ok().build();
            if ( validation.getStatus() == Status.UNAUTHORIZED.getStatusCode() ) {
                serverConstants.removeToken(txn, authToken.username, authToken.tokenID);
                ...
                txn.put(post);
                txn.commit();
                LOG.fine("Pin post: " + authToken.username + " " + pin + "ned post from the community with id " + communityID + ".");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response likePost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        String like = data.isLiked ? "like" : "unlike";
        LOG.fine("Like post: " + authToken.username + " attempted to " + like + " post with id " + data.postID + " from the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            var validation = Validations.checkLikePostValidation(user, community, post, member, likeRelation, token, authToken, data);
            //TODO: make validations
            validation = Response.ok().build();
            if ( validation.getStatus() == Status.UNAUTHORIZED.getStatusCode() ) {
                serverConstants.removeToken(txn, authToken.username, authToken.tokenID);
                ...
                txn.commit();
                LOG.fine("Like post: " + authToken.username + " " + like + "d post from the community with id " + communityID + ".");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    public Response dislikePost(@HeaderParam("authToken") String jsonToken, @PathParam("communityID") String communityID, PostData data) {
        AuthToken authToken = g.fromJson(jsonToken, AuthToken.class);
        String dislike = data.isDisliked ? "dislike" : "undislike";
        LOG.fine("Dislike post: " + authToken.username + " attempted to " + dislike + " post with id " + data.postID + " from the community with id " + communityID + ".");
        Transaction txn = datastore.newTransaction();
        try {
            Entity user = serverConstants.getUser(txn, authToken.username);
            Entity community = serverConstants.getCommunity(txn, communityID);
            ...
            var validation = Validations.checkDislikePostValidation(user, community, post, member, dislikeRelation, token, authToken, data);
            //TODO: make validations
            validation = Response.ok().build();
            if ( validation.getStatus() == Status.UNAUTHORIZED.getStatusCode() ) {
                serverConstants.removeToken(txn, authToken.username, authToken.tokenID);
                ...
                post = Entity.newBuilder(post.getKey())
                    ...
                    .set("lastEdit", post.getTimestamp("lastEdit"))
                    .set("likes", post.getLong("likes"))
                    .set("dislikes", data.isDisliked ? post.getLong("dislikes") + 1L : post.getLong("dislikes") - 1L)
                    .set("comments", post.getLong("comments"))
                    .set("isLocked", post.getBoolean("isLocked"))
                    .set("isPinned", data.isPinned)
                    .set("pinDate", data.isPinned ? Timestamp.now() : Timestamp.MIN_VALUE)
                ...
                txn.commit();
                LOG.fine("Dislike post: " + authToken.username + " " + dislike + "d post from the community with id " + communityID + ".");
